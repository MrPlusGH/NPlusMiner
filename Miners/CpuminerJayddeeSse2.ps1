...
$Commands | Get-Member -MemberType NoteProperty | Select -ExpandProperty Name | ForEach {
    ...
    [PSCustomObject]@{
        Type = "CPU"
        Path = $Path
        Arguments = "-t $($ThreadCount) -b $($Variables.CPUMinerAPITCPPort) -a $_ -o $($Pools.(Get-Algorithm $_).Protocol)://$($Pools.(Get-Algorithm $_).Host):$($Pools.(Get-Algorithm $_).Port) -u $($Pools.(Get-Algorithm $_).User) -p $($Pools.(Get-Algorithm $_).Pass)$($Commands.$_)"
