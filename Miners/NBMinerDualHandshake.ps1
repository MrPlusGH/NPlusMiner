    ...
    $Pools.Ethash | foreach {
        ...
        $Pools.handshake | foreach {
            ...
            If (($Pool_Ethash.Host -like "*nicehash*" -or $Pool_Ethash.Host -like "*miningpoolhub*")) {
                $Arguments = " -a hns_ethash --no-watchdog --temperature-limit 95 -di 4 -d $($Config.SelGPUCC) --api 127.0.0.1:$($Variables.NVIDIAMinerAPITCPPort) -o stratum+tcp://$($Pool_eaglesong.Host):$($Pool_eaglesong.Port) -u $($Pool_eaglesong.User):$($Pool_eaglesong.Pass) -do nicehash+tcp://$($Pool_Ethash.Host):$($Pool_Ethash.Port) -du $($Pool_Ethash.User):$($Pool_Ethash.Pass)"
            } else {
                $Arguments = " -a hns_ethash --no-watchdog --temperature-limit 95 -di 4 -d $($Config.SelGPUCC) --api 127.0.0.1:$($Variables.NVIDIAMinerAPITCPPort) -o stratum+tcp://$($Pool_eaglesong.Host):$($Pool_eaglesong.Port) -u $($Pool_eaglesong.User):$($Pool_eaglesong.Pass) -do stratum+tcp://$($Pool_Ethash.Host):$($Pool_Ethash.Port) -du $($Pool_Ethash.User):$($Pool_Ethash.Pass)"
            }
