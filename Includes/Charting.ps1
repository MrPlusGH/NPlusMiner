...
Switch ($Chart) {
    "Front7DaysEarnings" {
        ...
        # title 
           ...
           $chartarea.AxisY.labelAutoFitStyle = $chartarea.AxisY.labelAutoFitStyle - 4
           # $chartarea.AxisY.IntervalAutoMode = 0
           $chartarea.AxisY.Interval = [math]::Round(($datasource | group date | % {($_.group.DailyEarnings | measure -sum).sum} | measure -maximum).maximum *1000 / 4, 2)
           $chart1.ChartAreas.Add($chartarea)
...
# save chart
   ...
   $chart1
 | ForEach-Object {$chart1.Series[$Pool].Points.addxy( $_.Pool , ("{0:N3}" -f ([Decimal]$_.DailyEarnings*1000))) | Out-Null }
           # $Chart1.Series["Data"].Points.DataBindXY($datasource.pool, $datasource.DailyEarnings)
        }
    }
}
