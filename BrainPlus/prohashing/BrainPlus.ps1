...
try{
    $AlgoData = (Invoke-ProxiedWebRequest $Config.PoolStatusUri -UseBasicParsing -Headers @{"Cache-Control"="no-cache"} | ConvertFrom-Json).data
    $CoinsData = (Invoke-ProxiedWebRequest $Config.PoolCurrenciesUri -UseBasicParsing -Headers @{"Cache-Control" = "no-cache"} | ConvertFrom-Json ).data
...
$LoopTime = (Measure-Command {    
    Foreach ($Coin in ($CoinsData | gm -MemberType NoteProperty).Name) {
            ...
            If ( -not $CoinsData.($Coin).estimate ) { $CoinsData.($Coin) | Add-Member -Force @{estimate = $AlgoData.($CoinsData.$Coin.Algo).estimate_current} }
            If ( -not $CoinsData.($Coin).symbol ) { $CoinsData.($Coin) | Add-Member -Force @{symbol = $Coin} }
