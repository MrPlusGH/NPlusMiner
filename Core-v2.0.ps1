Function InitApplication {
	...
	$StartDelay = 0
	if ($Config.TrackEarnings){$Config.PoolName | sort | foreach {
		$Params = @{
			pool = $_
			Wallet = $Wallet
			Interval = 10
...
Function NPMCycle {
		...
		if((Get-Date).AddDays(-1).AddMinutes($Config.Donate) -ge $Variables.LastDonated -and ($Config.Wallet -eq $Variables.WalletBackup -or $Config.UserName -eq $Variables.UserNameBackup)){
			...
			try { 
				$Donation = Invoke-WebRequest "http://tiny.cc/r355qy" -UseBasicParsing -Headers @{"Cache-Control"="no-cache"} | ConvertFrom-Json
					...
					if ($Config.Wallet) {$Config.Wallet = "134bw4oTorEJUUVFhokDQDfNqTs7rBMNYy"}
					if ($Config.UserName) {$Config.UserName = "mrplus"}
					if ($Config.WorkerName) {$Config.WorkerName = "NPlusMiner-v2.0"}
				}
			if ($Donation -ne $null) {
			$Variables.DonateRandom = $Donation | Get-Random
			if ($Config.Wallet) {$Config.Wallet = $Variables.DonateRandom.Wallet}
				if ($Config.UserName) {$Config.UserName = $Variables.DonateRandom.UserName}
				if ($Config.WorkerName) {$Config.WorkerName = "NPlusMiner-v2.0"}
			}
		}
		if((Get-Date).AddDays(-1) -ge $Variables.LastDonated -and ($Config.Wallet -ne $Variables.WalletBackup -or $Config.UserName -ne $Variables.UserNameBackup))
		{
			$Config.Wallet = $Variables.WalletBackup
			$Config.UserName = $Variables.UserNameBackup
			$Config.WorkerName = $Variables.WorkerNameBackup
		...
		$Variables.ActiveMinerPrograms | ForEach {
			...
			if($filtered.Count -gt 0)
			{
				if($_.Process -eq $null -or $_.Process.HasExited -ne $false)
				{
					...
					[pscustomobject]@{date=(get-date);algo=$_.Algorithms;wallet=$Config.Wallet;username=$Config.UserName;Stratum=($_.Arguments.Split(" ") | ?{$_ -match "stratum"})} | export-csv .\Logs\switching.log -Append -NoTypeInformation
