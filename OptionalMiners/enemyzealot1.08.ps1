...
$Uri = "http://nemos.dx.am/opt/nemos/enemyz1.08.7z"

$Commands = [PSCustomObject]@{
    #"polytimos" = " -d $($Config.SelGPUCC) --api-remote --api-allow=0/0" #Polytimos
    #"hsr" = " -d $($Config.SelGPUCC) --api-remote --api-allow=0/0" #Hsr
    #"phi" = " -d $($Config.SelGPUCC) -i 20" #Phi(spmodphi faster)
    #"bitcore" = " -d $($Config.SelGPUCC) -i 20" #Bitcore(spmodbitcore faster very close tho)
    "x16r" = " -d $($Config.SelGPUCC) -N 180 -i 20" #X16r(fastest poolparty very close)
    "x16s" = " -d $($Config.SelGPUCC) -N 180 -i 20" #X16s(fastest)
