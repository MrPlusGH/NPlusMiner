...
$Commands | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name | ForEach-Object {
	...
	$Algo = Get-Algorithm($_)
        ...
        Type      = "AMD"
        Path      = $Path
        Arguments =  "--no-cpu --auto --doublecheck --mport $($Variables.AMDMinerAPITCPPort) -o stratum+tcp://$($Pools.($Algo).Host):$($Pools.($Algo).Port) -u $($Pools.($Algo).User) -p $($Pass) --nicehash --stakjson --any $($Commands.$_)"
